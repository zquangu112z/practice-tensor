...
def nextBatch(batchSize, batchIdx):
...
losses = [tf.nn.softmax_cross_entropy_with_logits(
    logits=logits, labels=labels) for logits, labels in zip(logits_series, labels_series)]   # 20 ,100,1

# print('losses: ', len(losses))
# print('losses[0]: ', losses[0].shape)
# print('losses[1]: ', losses[1].shape)
# print('losses[1][0]: ', losses[1][0])
...
with tf.Session() as sess:
    ...
    for epoch_idx in range(num_epochs):
        ...
        for batch_idx in range(num_batches):
            ...
            batchX, batchY, batchSegLen = nextBatch(batch_size, batch_idx)
